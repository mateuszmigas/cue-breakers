...
import init_contracts_wasm from "@wasm/contracts/lib_contracts";
import contracts_wasm_url from "@wasm/contracts/lib_contracts_bg.wasm?url";

const loadWasmModules = () =>
  Promise.all([
    init_game_logic(game_logic_url),
    init_physics_wasm(physics_wasm_url),
    init_contracts_wasm(contracts_wasm_url),
  ]);
